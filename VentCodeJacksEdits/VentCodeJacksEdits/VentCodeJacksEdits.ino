...
// Location for DHLR differential pressure transducer libraries:
// https://www.arduinolibraries.info/libraries/all-sensors-dlhr
...
#define FASTPRESSUREMEASUREMENT   // print out every fast pressure measured in expiration
...
int measured_peep           = 0;          // lowest value recorded during exhalation BEFORE a trigger    
...
int ReadFlowPressureSensor()
{
  ...
  /*--  UF DIGITAL SENSOR
    ...
    gagePressure.startMeasurement();
       ...
       // gagePressure.startMeasurement();  needed again???
    gagePressure.readData(true);
     if (!isnan(gagePressure.pressure))
    {
    pdiff = gagePressure.pressure;
    }
...
float CalculateInstantFlow()
{
  ...
  if (delP < 0)
    ...
    correction = 1.0;
  // del P we are going to make be Pascals.
  // For the UF flow sensor, values are returned in Pascals.
  // For delp <5 Pa,   Flow in l/minute =  2.8 * Pa
  // For delp >4 Pa,   Flow in l/minute =  5.2 * SQRT(Pa)
  //
  // For Gordon's home system with a much less sensitive
  // transducer and a homemade obstruction....
  //
...
/*
   ...
   a)  measures the current_pressure at each slide  and compares to find peakinspiratorypressure
   b)  checks for any pressure greater than vent_pressure_limit
   c)  verifies that SOMETIME during inspiration the current_pressure reached higher than low pressure limit
   d)  sets the measured_peep to the peakinsspiratorypressure right at the beignning of exhalation
       (this gets ratcheted down during the remaining expiratory slices) 
*/
...
void check_pressure_limits() {
  ...
  if ( current_phase == (cut_off + 1)) {
    // WE ARE RIGHT AT THE BEGINNNING OF EXHALATION

    // Set the presumed measured_peep and let it get decremented down by code in vent_slice
    measured_peep = peakinspiratorypressure;  // start high and work down. 
...
void vent_slice() {
  ...
  // **** DEAL WITH FAST REQUIRED MEASUREMENTS IN EXHALATION FIRST *****
  ...
  #ifdef  FASTPRESSUREMEASUREMENT
  Serial.print(",");
  Serial.print(p-atmospheric_pressure);
  #endif
  

    if  ((p + 1) < (atmospheric_pressure))  { // this needs to be adjusted for peep!!!!
                                      // Note that I tried to set it low enough that expiratory bumps wwon't set it off. 
                                      // If we got here--- the patient is trying to take a breath and has reduced the pressure
                                      // considerably below PEEP setting (we cann't use measured_peep  because it is being calculated
      Serial.print("\n\n**ASSISTED BREATH**\n\n");
  ...
  if (current_phase > cut_off) // Note that if we have jammed an inspiratio to start, this will be avoided
  {
    ...
    // UPDATE MEASURED_PEEP
    //if(current_pressure < measured_peep)  measured_peep = current_pressure;   // measured the bottom of the RINGING!
     measured_peep = current_pressure;   // only set during expiration!  
  ...
  if (current_phase > cut_off) {
    ...
    */
    ...
    exLitersPerCycle = exLitersPerCycle  +   (  CalculateInstantFlow() * 30 / ( (float) (beats_per_minute *  MAX_PHASES))  )  ;
