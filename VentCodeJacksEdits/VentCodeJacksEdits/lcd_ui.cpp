...
void lcd_graph_update(){
 ...
 // GLG
  ...
  for (int i=0; i < 2-strlen(buff); i++)
    ...
    // peakinspiratory pressure displayed top left
    itoa(peakinspiratorypressure,buff, 10);  // print the peak inspiratory pressure
    strcat(buff,"cm ");

    // exhaled tidal volume displayed middle top
    itoa(exmLPerCycle,buff2,10);  // print out the expiratory tidal volume detected
    strcat(buff,buff2);   // add it to the display
    strcat(buff,"mL ");

    // measured_peep displayed third on the top line
    itoa(measured_peep,buff2,10);  // print the measured_peep into the buff2
    strcat(buff,buff2);  // add it to the display
    strcat(buff,"cm");
    lcd.print(buff);
